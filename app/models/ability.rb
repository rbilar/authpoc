...
class Ability
  ...
  def initialize(user)
    ...
    # Invoice - owner
    #can :manage, Invoice, user_id: user.id if permission(user, 'all.invoice.owner')
    #can :read  , Invoice, user_id: user.id if permission(user, 'all.invoice.owner.read')
    #can :create, Invoice, user_id: user.id if permission(user, 'all.invoice.owner.create')
    #can :edit  , Invoice, user_id: user.id if permission(user, 'all.invoice.owner.update')
    #can :delete, Invoice, user_id: user.id if permission(user, 'all.invoice.owner.delete')
    ...
    # User - owner
    can :manage, User, user_id: user.id if permission(user, 'all.user.owner')
    can :read  , User, user_id: user.id if permission(user, 'all.user.owner.read')
    can :create, User, user_id: user.id if permission(user, 'all.user.owner.create')
    can :edit  , User, user_id: user.id if permission(user, 'all.user.owner.update')
    can :delete, User, user_id: user.id if permission(user, 'all.user.owner.delete')
  end
  ...
  #     role.role_permissions.where("permission LIKE '#{name}%'").count > 0
  #   end
  # end
